...
function build(type,data){
 ...
 const s={type,data,selected:0,compact:storage.get(`nitrate-preview-${type}`)==='compact',busy:false,seen:new Set(),animations:[]};
 states.push(s);
 const section=document.createElement('section');section.innerHTML=`<div class="label"><b>${seriesMode?'Séries':'Films'}</b><span>${seriesMode?'Grande carte ↔ Vue d’ensemble':'Deux colonnes ↔ Trois colonnes'}</span></div><div class="phone ${type}" aria-label="Aperçu ${seriesMode?'Séries':'Films'}"><div class="library"><div class="status"><span>9:41</span>${icon('battery-full')}</div><div class="app-head"><img class="brand" src="../action-menus/assets/wordmark.svg" alt="Nitrate"><span aria-label="Collection">${seriesMode?icon('library-big'):'<img class="collection" src="../action-menus/assets/films-seen.svg" alt="Films vus">'}</span><span style="margin-left:12px" aria-label="Notifications">${icon('bell')}</span></div><div class="tabs" aria-hidden="true"><span class="active">${seriesMode?'À voir':'Ma liste'}</span><span>${seriesMode?'À venir':'Sorties'}</span></div><div class="heading"><div><h2>${seriesMode?'À reprendre.':'Ta prochaine séance.'}</h2><p>${seriesMode?'6 séries dans ta rotation.':'6 films à découvrir.'}</p></div><div class="view-toggle" role="group" aria-label="Disposition ${type}"><button data-layout="large" aria-label="${seriesMode?'Grande carte Séries':'Grandes affiches Films'}" title="${seriesMode?'Grande carte':'Grandes affiches'}">${icon(seriesMode?'panel-top':'columns-2')}</button><button data-layout="compact" aria-label="Vue d’ensemble ${seriesMode?'Séries':'Films'}" title="Vue d’ensemble">${icon('grid-2x2')}</button></div></div><div class="scroller"><div class="stage"></div></div><nav class="nav" aria-label="Navigation illustrative">${[['tv','Séries'],['clapperboard','Films'],['scan-search','Explorer'],['user-round','Profil']].map(([i,t],idx)=>`<button tabindex="-1" aria-disabled="true" class="${idx===(seriesMode?0:1)?'active':''}">${icon(i)}<span>${t}</span></button>`).join('')}</nav></div><section class="detail" role="dialog" aria-modal="true" aria-label="Fiche" hidden></section><div class="toast" role="status" hidden></div></div><p class="caption">${seriesMode?'Le prochain épisode de chaque série devient visible en un coup d’œil. La grande carte reste disponible.':'Des affiches plus petites pour parcourir la collection. Ouvre un film : son affiche prend sa place dans la fiche.'}</p>`;
 document.querySelector('#previews').append(section);s.phone=section.querySelector('.phone');s.stage=section.querySelector('.stage');s.scroll=section.querySelector('.scroller');s.library=section.querySelector('.library');s.detail=section.querySelector('.detail');
...
async function morph(s,change){
 ...
 const time=duration(),factor=time/640;
 ...
 const jobs=arts.map((el,i)=>{
   ...
   const delay=Math.min(65,rank*22)*factor;
   s.cards[i].style.zIndex=i===s.selected?6:2;
   // Animate the clipping rectangle, with a uniform image crop at both ends.
   // Translation moves the poster; changing width/height never scales its text.
   return animate(el,[
     {transform:`translate(${a.rect.left-b.rect.left}px,${a.rect.top-b.rect.top}px)`,width:`${a.rect.width}px`,height:`${a.rect.height}px`,borderRadius:a.radius,...posterFrame(a.rect.width,a.rect.height,i)},
     {transform:'translate(0,0)',width:`${b.rect.width}px`,height:`${b.rect.height}px`,borderRadius:b.radius,...posterFrame(b.rect.width,b.rect.height,i)}
   ],{duration:time,delay,easing,fill:'both'});
 });
 ...
 newCopy.forEach(el=>jobs.push(animate(el,[{opacity:0,transform:'translateY(7px)'},{opacity:1,transform:'translateY(0)'}],{duration:230*factor,delay:340*factor,easing,fill:'both'})));
...
function setLayout(s,compact){if(s.busy){s.pendingCompact=compact;return}if(s.compact===compact)return;morph(s,()=>{s.compact=compact;storage.set(`nitrate-preview-${s.type}`,compact?'compact':'large')});}
...
build('series',series);build('films',movies);lucide.createIcons();document.querySelector('#replay').onclick=()=>states.forEach(s=>setLayout(s,!s.compact));let resizeTimer;window.addEventListener('resize',()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>states.forEach(s=>{s.animations.forEach(a=>a.finish());layout(s)}),100)});
