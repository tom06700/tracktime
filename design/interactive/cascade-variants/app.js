...
async function morph(s,change){
 ...
 const jobs=arts.map((el,i)=>{
   ...
   const delay=(s.variant.id==='cascade-depth'?Math.floor(rank/2)*95+(rank%2)*65:rank*s.variant.stagger)*factor;
 ...
 if(s.variant.id!=='cascade-depth')jobs.push(animate(s.stage,[{filter:'blur(0px) brightness(1)'},{filter:`blur(${s.variant.blur}px) brightness(${s.variant.brightness})`,offset:.43},{filter:'blur(0px) brightness(1)'}],{duration:time+5*s.variant.stagger*factor,easing:'ease-in-out',fill:'both'}));
 ...
 newCopy.forEach(el=>jobs.push(animate(el,[{opacity:0,transform:'translateY(7px)'},{opacity:1,transform:'translateY(0)'}],{duration:230*factor,delay:s.variant.caption*factor,easing,fill:'both'})));
...
const variants=[
{id:'cascade',name:'02 · Cascade ciné',subtitle:'L’originale que tu as aimée',description:'Même mouvement qu’avant : une cascade régulière, un bref flou lumineux, puis une arrivée nette.',source:'Référence · Codrops',url:'https://tympanus.net/Tutorials/GridLayoutTransitions/index2.html',steps:'ORIGINALE · CASCADE ET MISE AU POINT',ms:780,stagger:48,blur:3,brightness:1.14,caption:560},
{id:'cascade-soft',name:'02A · Cascade velours',subtitle:'Plus douce et plus discrète',description:'Les cartes glissent presque ensemble. Le flou est plus léger, la lumière plus douce et le mouvement plus court.',source:'Même inspiration · variation Nitrate',url:'https://tympanus.net/Tutorials/GridLayoutTransitions/index2.html',steps:'FLOU LÉGER · GLISSEMENT SOUPLE',ms:660,stagger:24,blur:1.1,brightness:1.05,caption:420},
{id:'cascade-depth',name:'02B · Cascade en profondeur',subtitle:'Une vague d’affiches qui se posent',description:'Les affiches prennent un léger recul pendant le déplacement, puis se posent par vagues. Chaque carte retrouve sa netteté séparément.',source:'Même inspiration · variation Nitrate',url:'https://tympanus.net/Tutorials/GridLayoutTransitions/index2.html',steps:'RECUL DISCRET · VAGUE · NETTETÉ',ms:760,stagger:48,blur:0,brightness:1,caption:540}
